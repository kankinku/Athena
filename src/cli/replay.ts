/**
 * `athena replay <session-id>` — read-only playback of a session's conversation.
 */
...
const sessionId = Args.text({ name: "session-id" }).pipe(
  Args.withDescription("Session ID to replay (use 'athena sessions' to find IDs)"),
);
...
export const replay = Command.make(
  ...
  ({ sessionId, raw }) =>
    Effect.promise(async () => {
      ...
      if (!session) {
        console.error(`Session "${sessionId}" not found. Use 'athena sessions' to list sessions.`);
        process.exit(1);
      }
      ...
      for (const msg of messages) {
        ...
        switch (msg.role) {
          case "user":
            ...
            break;
          case "assistant":
            console.log(`\x1b[36m┌─ Athena \x1b[2m${time}\x1b[0m`);
