...
import { createRuntime, type AthenaRuntime } from "./init.js";
...
export function App({
  ...
  const [runtime, setRuntime] = useState<AthenaRuntime | null>(null);
  ...
  if (error) {
    return (
      <Box padding={1}>
        <Text color="red">Failed to start Athena: {error}</Text>
      </Box>
    );
  }
  ...
  if (!runtime) {
    return (
      <Box padding={1}>
        <Text color="yellow">Starting Athena...</Text>
      </Box>
    );
  }
