...
export class Notifier {
  ...
  private async sendToChannel(
    ...
    try {
      ...
      process.stderr.write(
        `[athena] notification error (${channel.type}): ${formatError(err)}\n`,
      );
    }
  }
  ...
  private sendDesktop(payload: NotificationPayload): Promise<void> {
    ...
    return new Promise<void>((resolve) => {
      exec(cmd, (err) => {
        if (err) {
          process.stderr.write(
            `[athena] desktop notification error: ${err.message}\n`,
          );
        }
        resolve();
      });
  ...
  private async sendWebhook(
    ...
    try {
      ...
      process.stderr.write(
        `[athena] webhook error (${channel.url}): ${formatError(err)}\n`,
      );
    } finally {
    ...
    return new Promise<void>((resolve) => {
      exec(
        channel.command,
        {
          env: {
            ...process.env,
            ATHENA_EVENT: payload.event,
            ATHENA_TITLE: payload.title,
            ATHENA_BODY: payload.body,
          },
        },
        (err) => {
          if (err) {
            process.stderr.write(
              `[athena] command notification error: ${err.message}\n`,
            );
          }
          resolve();
        },
