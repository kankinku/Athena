...
export class ExperimentBrancher {
  ...
  async createBranch(
    ...
    const sanitized = experimentName
      ...
      .replace(/-+/g, "-")
      .replace(/^-|-$/g, "");
    const branchName = `athena/exp-${sanitized}-${Date.now().toString(36)}`;
  ...
  /**
   * List all athena experiment branches in a repo.
   */
  async listBranches(
    machineId: string,
    repoPath: string,
    ...
    const result = await this.executor.exec(
      machineId,
      `cd ${shellQuote(repoPath)} && git branch --list 'athena/exp-*' --format='%(refname:short)'`,
    );
