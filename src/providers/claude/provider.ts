...
export class ClaudeProvider implements ModelProvider {
    ...
    if (this.authMode === "cli") {
      if (attachments?.length) {
        process.stderr.write(
          "[athena] Warning: file attachments are not supported in Claude CLI mode. Switch to API mode (--claude-mode api) to send files.\n",
        );
      }
  ...
  /** Strip the MCP server prefix (e.g. "mcp__athena__remote_exec" → "remote_exec") */
  ...
  private buildMcpServer(tools: ToolDefinition[]) {
    ...
    return createSdkMcpServer({ name: "athena-tools", tools: mcpTools });
  }
