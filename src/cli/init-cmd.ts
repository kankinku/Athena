/**
 * `athena init` — initialize a project with a athena.json config file.
 */
...
export const initCmd = Command.make(
  ...
  ({ provider, model, machine }) =>
    Effect.promise(async () => {
      ...
      const configPath = join(dir, "athena.json");

      if (existsSync(configPath)) {
        console.log(`athena.json already exists in ${dir}`);
        console.log("Edit it directly or delete it and re-run 'athena init'.");
        return;
      }
      ...
      console.log(`Created athena.json in ${dir}`);
