...
function cmdHub(args: string[], ctx: CommandContext): void {
  ...
  if (subCmd === "connect") {
    const url = args[1];
    const agentName =
      args[2] ?? `athena-${Math.random().toString(36).slice(2, 8)}`;
    if (!url) {
      addMessage(
        "system",
        "Usage: /hub connect <url> [agent-name]",
    ...
    HubClient.selfRegister(cleanUrl, agentName).then(
      (result) => {
        ...
        if (executor) {
          ...
          addMessage(
            "system",
            `Registered as "${result.id}". Restart Athena to activate hub tools.`,
          );
        }
      },
      (err) =>
  ...
  if (subCmd === "disconnect") {
    ...
    addMessage(
      "system",
      "AgentHub config removed. Restart Athena to remove hub tools.",
    );
    return;
  }
