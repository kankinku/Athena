...
Athena는 단발성 답변 시스템이 아니다. 목표를 받으면 스스로 다음 개선을 계획하고, 실행하고, 결과를 평가하고, 그 결과를 바탕으로 다음 개선 방향을 다시 설계하는 시스템이다.
...
연구는 이 루프를 위한 근거 수집 수단이다. 논문, URL, 문서, 저장소 상태, 과거 실험 기록은 모두 다음 개선을 더 잘 설계하기 위한 입력이다.
